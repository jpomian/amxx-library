...
public plugin_init()
{
	...
	new pcvar = create_cvar("afk_global_messages_away_time", "60", FCVAR_NONE, "Time in seconds that a client can be AFK before being able to trigger AFK global messages^nAFK gobal messages are displayed to a team when all the enemies of the opposing team are AFK^nSet this cvar to 0 to disable the display of AFK global messages", .has_min = true, .min_val = 0.0)
	...
	pcvar = create_cvar("afk_bomb_action_time", "25", FCVAR_NONE, "Time in seconds that a client can have the bomb when being AFK", .has_min = true, .min_val = 0.1)
	...
	pcvar = create_cvar("afk_bomb_management_immunity", "1", FCVAR_NONE, "(0|1) - Disable/Enable admin immunity for AFK bomb management", .has_min = true, .min_val = 0.0, .has_max = true, .max_val = 1.0)
...
public afk_manager_loop(ent)
{
	...
	if ( GlobalMessagesAwayTime > 0.0 )
	{
		if ( cts_afk > 0 && cts_not_afk == 0 && terrorists_not_afk > 0 )
		{
			if ( ColoredMessages )
			{
				set_hudmessage(0, 50, 255, 0.02, 0.688, 0, 0.0, LoopFrequency, 0.0, 0.0)
			}
			else
			{
				set_hudmessage(255, 255, 255, 0.02, 0.688, 0, 0.0, LoopFrequency, 0.0, 0.0)
			}
		...
		else if ( terrorists_afk > 0 && terrorists_not_afk == 0 && cts_not_afk > 0 )
		{
			if ( ColoredMessages )
			{
				set_hudmessage(255, 50, 0, 0.02, 0.688, 0, 0.0, LoopFrequency, 0.0, 0.0)
			}
			else
			{
				set_hudmessage(255, 255, 255, 0.02, 0.688, 0, 0.0, LoopFrequency, 0.0, 0.0)
			...
			for ( i = 0; i < player_count; i++ )
			{
				show_hudmessage(players[i], "%l", "ALL_TERRORISTS_AFK")
			}
		}
	}
}
