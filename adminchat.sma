...
#define MAX_CLR 9
#define is_user_adm(%0) get_user_flags(%0) & ADMIN_BAN
#define VIP ADMIN_LEVEL_H
 
new g_Colors[MAX_CLR][] = {"COL_WHITE", "COL_RED", "COL_GREEN", "COL_BLUE", "COL_YELLOW", "COL_MAGENTA", "COL_CYAN", "COL_ORANGE", "COL_PURPLE"}
new g_Values[MAX_CLR][] = {{255, 255, 255}, {255, 0, 0}, {0, 255, 0}, {0, 0, 255}, {255, 255, 0}, {255, 0, 255}, {0, 255, 255}, {227, 96, 8}, {140, 26, 255}}
...
public cmdSayChat(id)
{
	...
	log_to_file("addons/amxmodx/logs/logi_czatadmina.log", "%s : %s",name,message[i+n])
...
public cmdSayAdmin(id)
{
	...
	if (is_user_adm(id))
		format(message, 191, "(A) %s :  %s", name, message[1])
	else if(get_user_flags(id) & VIP)
		format(message, 191, "(V) %s :  %s", name, message[1])
	else
		format(message, 191, "(G) %s :  %s", name, message[1])
	...
	for (new i = 0; i < inum; ++i)
		...
		if (players[i] != id && get_user_flags(players[i]) & g_AdminChatFlag)
			ColorChat(players[i],GREEN,"%s",message)
	
	ColorChat(id,GREEN,"%s",message)
	log_to_file("addons/amxmodx/logs/Logi_u@.log", message)
