...
public plugin_init()
{
    ...
    register_clcmd("say mapy", "clcmd_mapslist");
    register_clcmd("say /mapy", "clcmd_mapslist");
    register_clcmd("say zm_", "clcmd_mapslist");
...
nominate_map(id, map[])
{
    ...
    if(map_info[MaxPlayers] != 0)
        client_print_color(0, id, "%s^3 %L", g_sPrefix, LANG_PLAYER, "MAPM_NOM_MAP2", name, map, map_info[MinPlayers], map_info[MaxPlayers]);
    else
...
show_nomination_menu(id, Array:maplist, custom_title[] = "")
{
    ...
    new map_info[MapStruct], item_name[MAPNAME_LENGTH + 16], item_name_alt[MAPNAME_LENGTH + 16], block_count, size = ArraySize(maplist);
    ...
    for(new i = 0, index, nom_index; i < size; i++) {
        ...
        if(block_count) {
            ...
            formatex(item_name, charsmax(item_name), "%s", map_info[Map]);
            formatex(item_name_alt, charsmax(item_name_alt), "%s (\y%d\w-\y%d\w)", map_info[Map], map_info[MinPlayers], map_info[MaxPlayers]);
            menu_additem(menu, map_info[MaxPlayers] != 0 ? item_name_alt : item_name);
        }
    }
