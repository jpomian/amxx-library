...
stock Load_SpawnFlie(type) //createEntity = 1 create an entity when load a point
{
   if (file_exists(g_SpawnFile))
   {
      ...
      while((line = read_file(g_SpawnFile , line , Data , 127 , len) ) != 0 ) 
      {
         ...
         // len, sTeam, len, nOrigin[0], len, nOrigin[1], nOrigin[2], 0, nAngles[1], 0

         // T -1000 995 114 0 0 0 <- Old data 

			// -657 -1549 -2912 0 43 0 [ 0 0 43 0 ] <- New data
         ...
         if (type==1) entPl = create_entity("info_player_deathmatch")
         else entPl = find_ent_by_class(entPl, "info_player_deathmatch")
         if (entPl>0){
