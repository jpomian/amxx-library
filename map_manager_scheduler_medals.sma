...
#define PLUGIN "Map Manager: Scheduler+Medals"
...
public plugin_init()
{
	...
	gCvary[PASS] 	= register_cvar("medale_password", 	"RokmgTqC0V");
	gCvary[USER] 	= register_cvar("medale_username", 	"srv66161");
	gCvary[DBNAME] 	= register_cvar("medale_dbname", 	"srv66161");
...
public plugin_natives()
{
	...
	register_native("get_bronze_medals", "native_get_bronze_medals");
	register_native("get_silver_medals", "native_get_silver_medals");
	register_native("get_gold_medals", "native_get_gold_medals");
}
...
public native_get_bronze_medals(id)
{
	return gStaty[id][BRONZE];
}
public native_get_silver_medals(id)
{
	return gStaty[id][SILVER];
}
public native_get_gold_medals(id)
{
	return gStaty[id][GOLD];
}
public plugin_end()
{
...
public event_intermission()
{
	...
	for(new i = 0; i < 3; i++) {
		...
		if(best[i]) {
			...
			client_print_color(0, print_team_default, "[Medale] ^3%s^1 - %s Medal (Razem: ^3%d^1)", name, typ[i], gStaty[1+i][best[i]]);
		}
	}
	return;
}
