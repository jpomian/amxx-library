...
public plugin_precache() {
	...
	//
		...
		cvar[CVAR_SQL_HOST] = create_cvar("csstats_sql_host", "sql.pukawka.pl", FCVAR_PROTECTED, "MySQL host")
		cvar[CVAR_SQL_USER] = create_cvar("csstats_sql_user", "898035", FCVAR_PROTECTED, "MySQL user")
		cvar[CVAR_SQL_PASS] = create_cvar("csstats_sql_pass", "zmKOLOSEUM", FCVAR_PROTECTED, "MySQL user password")
		cvar[CVAR_SQL_DB] = create_cvar("csstats_sql_db", "898035_csstats", FCVAR_PROTECTED, "DB Name")
...
public EventHook_Damage(player)
{
	...
	if(0 <= last_hit < HIT_END && !is_user_bot(player))
	{
		Stats_SaveHit(attacker,player,damage_take,weapon_id,last_hit)
	}
	
	if(!alive && !is_user_bot(player)) {
