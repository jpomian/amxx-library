...
new const MotdTitle[ ] = "Osiagniecia";
...
public plugin_init( ) {
	...
	if( !iFile )
		SetFailState( "Niesukcesywne otwarcie pliku .cfg." );
	...
	register_clcmd( "say /achsweb",          "CmdAchievements" );
	register_clcmd( "say /playtime",       "CmdPlayedTime" );
	register_clcmd( "say /czas",     "CmdPlayedTime" );
	register_clcmd( "say /osiagnieciaweb", "CmdAchievements" );
...
public NativeAchievementProgress( const iPlugin, const iParams ) {
	...
	#if defined DEBUG
		ColorChat( 0, RED, "^1Postep Osiagniecia^3 #%i^1 -^4 ^"%s^" (%i)^1 -^3 Juz odblokowane!",
...
#if defined DEBUG
	ColorChat( 0, RED, "^1Postep Osiagniecia dla^3 #%i^1 -^4 ^"%s^" (%i)^1 - Postep:^4 %i^1 (%i/%i)",
...
#if defined USE_TUTOR
	UTIL_ShowTutor( id, TUTOR_RED, 6.0, "Postep Osiagniecia:^n%s (%i/%i)", szName, iProgress, iNeededToGain );
#else
	engclient_print( id, engprint_center, "^nPostep Osiagniecia:^n%s (%i/%i)", szName, iProgress, iNeededToGain );
#endif
}

AwardAchievement( const id, const iAchievement, const Achievement[ AchievementData ] ) {
#if defined USE_TUTOR
	UTIL_ShowTutor( id, TUTOR_GREEN, 8.0, "Osiagniecie Odblokowane:^n%s", Achievement[ Achv_Name ] );
#endif
	
	new szName[ 32 ];
	get_user_name( id, szName, 31 );
	
	ColorChat( 0, RED, "%s %s^1 zdobyl osiagniecie:^4 %s^1.", Prefix, szName, Achievement[ Achv_Name ] );
...
public CmdPlayedTime( const id ) {
	...
	ColorChat( id, RED, "%s^1 Grasz na tym serwerze od^4 %s", Prefix, szTime );
}
	...
	if( szSaid[ 0 ] == '/' || szSaid[ 0 ] == '.' )
	{
		...
		if( equali( szCmd[ 1 ], "stats" ) ) {
			...
			if( !g_iPlayerId[ iPlayer ] ) {
				ColorChat( id, RED, "%s^1 Nie mozesz zobaczyc informacji o tym graczu.", Prefix );
				return;
			}
			...
			if( !g_iTotalAchievs ) {	
				ColorChat( id, RED, "%s^1 Nie ma tu osiagniec.", Prefix );
				return;
			}
			...
			if( !g_iPlayerId[ iPlayer ] ) {
				ColorChat( id, RED, "%s^1 Nie mozesz zobaczyc pozycji tego gracza.", Prefix );
				return;
			}
			...
			if( iPlayer != id ) {
				...
				ColorChat( id, RED, "%s %s^1 : Zdobyte Osiagniecia:^4 %i/%i^1.^3 (%i%%).^1 Czas Gry:^4 %s godzin^1.",
					Prefix, szName, iCount, g_iTotalAchievs, iPercent, szTime );
			} else
				ColorChat( id, RED, "%s^1 Zdobyte Osiagniecia:^4 %i/%i^1.^3 (%i%%).^1 Czas Gry:^4 %s godzin^1.",
...
FindPlayer( id, const szArg[ ] ) {
	...
	if( iPlayer ) {
		if( iPlayer != find_player( "blj", szArg ) ) {
			ColorChat( id, RED, "%s^1 Do Twojego wyszukiwania pasuje pare wynikow.", Prefix );
	...
	if( !iPlayer ) {
		ColorChat( id, RED, "%s^1 Gracz o takiej nazwie nie zostal znaleziony.", Prefix );
...
public FwdHamPlayerSpawn( const id ) {
	if( is_user_alive( id ) ) {
		...
		UTIL_ShowTutor( id, TUTOR_BLUE, 4.0, "Przegrales %s godzin^nMasz %i z %i osiagniec^n^nNie daj sie zarazic.",
			szTime, UTIL_CountUnlocks( id ), g_iTotalAchievs );
		else if(get_user_team(id) == 1)
		UTIL_ShowTutor( id, TUTOR_RED, 4.0, "Przegrales %s godzin^nMasz %i z %i osiagniec^n^nRozszerz plage dalej.",
			szTime, UTIL_CountUnlocks( id ), g_iTotalAchievs );
	}
}
...
public NativeSetComponentBit( const iPlugin, const iParams ) {
	...
	if( get_playersnum( ) <= 2 ) {
#if defined USE_TUTOR
		UTIL_ShowTutor( id, TUTOR_RED, 4.0, "Postep nie jest zapisywany^nZe wzgledu na brak graczy." );
#endif
	...
	if( 0 == ( iBitMask & Progress[ Progress_Bits ] ) ) {
...
#if defined DEBUG
			ColorChat( 0, RED, "^1Element^4 %d^1 dla osiagniecia^3 %s^1 znaleziony. (gracz: %i)", iBitMask, Achievement[ Achv_Name ], id );
#endif
		...
		ColorChat( 0, RED, "^1Element^4 %d^1 dla osiagniecia^3 %s^1 znaleziony, lecz jest juz w posiadaniu. (gracz: %i)",
