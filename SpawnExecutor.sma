...
new g_Editing
new g_SpawnPlayer, g_EditPlayer
new Laser_Spr
...
public c_Main(id, menu, item)
{
   ...
   if (num==1 || num==11){
      format(fItem,255,"%L ( PL = %d ) ^n0.\y %L \r( PL = %d ) ^n>> %L", id, "MENU_ORIGIN_SPAWNS",g_SpawnPlayer, id,"MENU_EDIT_SPAWNS", g_EditPlayer, id,"MENU_NOTICE_SAVE")
...
public m_MainHandler(id, menu, item)
{
   ...
   switch(iChoice)
   {
      ...
      case 3:{ // Add T Spawn
         ...
         else if (CreateEditEntity(id,g_OffSet)==1){
            g_EditPlayer++
            client_cmd(id,"spk buttons/button9")
      ...
      case 4:{ // Add CT Spawn
         ...
         else if (CreateEditEntity(id,g_OffSet)==2){
            g_EditPlayer++
            client_cmd(id,"spk buttons/button9")
      ...
      case 7:{ // Save All Spawn To File
         if (Save_SpawnsFile()){
            ...
            client_print(0,print_chat,">> %L (PL = %d)",id,"MSG_SAVE_SPAWNSFLIE", g_EditPlayer)
         }else
...
public plugin_precache()
{
   ...
   format(g_SpawnFile, 255, "%s/%s.spawns.cfg",spawndir, MapName)
   ...
   if (Load_SpawnFile(1)) //load spawn file and create player spawn points
...
stock Load_SpawnFile(type) //createEntity = 1 create an entity when load a point
{
   if (file_exists(g_SpawnFile))
   {
      new entPl, randomCheck[63], i = 0
      new Data[128], len, line = 0
      new p_origin[3][8], p_angles[3][8]
      ...
      while((line = read_file(g_SpawnFile , line , Data , 127 , len) ) != 0 ) 
      {
         ...
         parse(Data, p_origin[0],7, p_origin[1],7, p_origin[2],7, p_angles[0],7, p_angles[1],7, p_angles[2], 7)
         
         // len, szClassname, len, nOrigin[0], len, nOrigin[1], nOrigin[2], 0, nAngles[1], 0

         // T -1000 995 114 0 0 0 <- Old data 

		   // -657 -1549 -2912 0 43 0 [ 0 0 43 0 ] <- New data (irrelenvat)
         ...
         randomCheck = (i % 2) == 1 ? "info_player_deathmatch" : "info_player_start";

         // log_amx("Generated for: %s", randomCheck)
         i++

         if (type==1)
            entPl = create_entity(randomCheck)
         else
            entPl = find_ent_by_class(entPl, randomCheck)
         if (entPl>0){
               entity_set_int(entPl,EV_INT_iuser1,1) // mark that create by map spawns editor
               entity_set_origin(entPl, origin)
               entity_set_vector(entPl, EV_VEC_angles, angles)
            }
      }

      log_amx("Total Spawnpoints: %i", i)
   ...
   if (!g_DeathCheck_end){
      ...
      if (equal(string,"worldspawn")){
         ...
         if (g_Editing){
            ...
            Point_WriteToFlie(g_DieFile,id,1)
         }
      }
   }
   return PLUGIN_CONTINUE
...
stock Save_SpawnsFile()
{
   ...
   get_mapname(mapname,31)
   format(line,127,"/* %s T=%d,CT=%d */ Map Spawns Editor Format File",mapname,g_EditPlayer,g_EditPlayer)
   write_file(g_SpawnFile, line, -1)

   new entity
   while ((entity = find_ent_by_class(entity, EDIT_CLASSNAME))){
      Point_WriteToFlie(g_SpawnFile,entity,1)
   }
...
stock Export_RipentFormatFile()
{
   ...
   new entity
   while ((entity = find_ent_by_class(entity, EDIT_CLASSNAME))){
      Point_WriteToFlie(g_EntFile,entity,2)
   }
   return 1
}

// store one entity data to file
stock Point_WriteToFlie(Flie[],entity,saveformat)
{
   new line[128],szClassname[32]
   ...
   if (saveformat==1){ // write for plugin using format
      format(line, 127, "%d %d %d %d %d %d", nOrigin[0], nOrigin[1], nOrigin[2], 0, nAngles[1], 0)
      write_file(Flie, line, -1)
   }
   else if (saveformat==2){ // write for ripent.exe format
      szClassname = random(1) == 0 ? "info_player_deathmatch" : "info_player_start"
      format(line, 127,"{^n  ^"classname^" ^"%s^"",szClassname)
      write_file(Flie, line , -1)
