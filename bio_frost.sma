...
public plugin_init()
{
	...
	pcv_price = register_cvar("fn_price","0");
	pcv_icon = register_cvar("fn_icon","0");
	...
	pcv_chill_duration = register_cvar("fn_chill_duration","4.0");
	pcv_chill_variance = register_cvar("fn_chill_variance","1.0");
	pcv_chill_speed = register_cvar("fn_chill_speed","60.0");
	pcv_freeze_maxchance = register_cvar("fn_freeze_maxchance","100.0");
	pcv_freeze_minchance = register_cvar("fn_freeze_minchance","100.0");
	pcv_freeze_duration = register_cvar("fn_freeze_duration","2.0");
...
public frostnade_explode(ent)
{
	...
	for(new target=1;target<=maxPlayers;target++)
	{
		...
		if(is_user_zombie(owner) || !is_user_zombie(target)) continue;
...
freeze_player(id,attacker,nadeTeam)
{
	...
	//give_user_knockbackimmunity(id, duration);
...
chill_player(id,attacker,nadeTeam)
{
	...
	// glowshell
	fm_set_rendering(id, kRenderFxGlowShell, rgb[0], rgb[1], rgb[2], kRenderNormal, floatround(GLOW_AMOUNT));
