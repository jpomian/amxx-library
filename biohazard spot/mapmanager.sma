...
public plugin_init()
{
	...
	g_pCvars[EXTENDED_MAX] = register_cvar("mapm_extended_map_max", "2");
	g_pCvars[EXTENDED_TIME] = register_cvar("mapm_extended_time", "20");//minutes
	g_pCvars[EXTENDED_ROUNDS] = register_cvar("mapm_extended_rounds", "3");//rounds
	
	#if defined FUNCTION_RTV
	g_pCvars[ROCK_MODE] = register_cvar("mapm_rtv_mode", "1");//0 - percents, 1 - players
	...
	g_pCvars[NOMINATION_DEL_NON_CUR_ONLINE] = register_cvar("mapm_nom_del_noncur_online", "1");//0 - disable, 1 - enable
	#endif
	...
	register_clcmd("say zm_", "Command_MapsList");
...
public plugin_natives()
{
	...
	register_native("has_vote_started", "Native_HasVoteStarted")
}
public Native_IsNightMode()
{
	return g_bNightMode;
}
public Native_HasVoteStarted()
{
	return g_bStartVote;
}
#endif
...
public VoteMenu(id)
{
	...
	if(g_bPlayerVoted[id] && get_pcvar_num(g_pCvars[SHOW_RESULT_TYPE]) == 2)
	{
		...
		set_hudmessage(218, 165, 32, 0.02, -1.0, 0, 6.0, 1.0, 0.1, 0.2, 4);
