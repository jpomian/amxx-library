...
{
    if(is_user_alive(id) && !cs_get_user_autoswitch(id))
    {
	    if(is_weapon_primary(cs_get_weapon_type(ent)) && pev_valid(g_weaponent[id]) && !is_user_zombie(id))
...
public bacon_item_deploy_post(ent)
{
	...
	if(is_user_alive(id)) 
	{
		...
		if(is_weapon_primary(weapon) || cs_get_user_shield(id) || is_user_zombie(id))
			fm_set_entity_visibility(g_weaponent[id], 0)
...
public event_infect(victim, attacker)
    fm_set_entity_visibility(g_weaponent[victim], 0)
