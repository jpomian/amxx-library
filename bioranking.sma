...
public plugin_init() {
	...
	//Cvary
	register_cvar("amx_biohazard_host", "localhost") 
	register_cvar("amx_biohazard_user", "root")
	register_cvar("amx_biohazard_pass", "root") 
	register_cvar("amx_biohazard_db", "db")
	//Polaczenie z baza
	set_task(1.0, "polacz")
...
public polacz()
{
	...
	get_cvar_string("amx_biohazard_host", Host, 31)
	get_cvar_string("amx_biohazard_user", User, 31)
	get_cvar_string("amx_biohazard_pass", Pass, 31)
	get_cvar_string("amx_biohazard_db", DB, 31) 
	...
	new Handle:Queries = SQL_PrepareQuery(hConn,"CREATE TABLE IF NOT EXISTS `statystyki` (name VARCHAR(64) NOT NULL, kill INT(10) NOT NULL DEFAULT 0, PRIMARY KEY(name))") 
	//Nazwa tabelki statystyki ;)
...
public load(id)
{
	...
	formatex(szTemp,charsmax(szTemp),"SELECT * FROM `statystyki` WHERE `name` = '%s'", nazwa_gracza[id]) // Wyciągamy z bazy nick gracza
...
public register_client(failstate, Handle:query, error[],errcode, data[], datasize)
{
	...
	else // Nie ma nic trzeba dodać gracza do bazy
	{
		...
		data[0] = id
		formatex(szTemp,charsmax(szTemp),"INSERT INTO `statystyki` (`name`, `kill`) VALUES ('%s','%d')", nazwa_gracza[id], stats) // Dodajemy do bazy dane nick i ilosc zabojstw
...
{       
	...
	formatex(szTemp,charsmax(szTemp),"UPDATE `statystyki` SET `kill` = '%d' WHERE `name` = '%s'",stats, nazwa_gracza[id]) // Zmienia wartosc zabojstw gdzie nick = nick_gracza
...
public rank(id)
{
	...
	get_user_stats(id, stats, bodyhits)
	new szTemp[512]
	format(szTemp,charsmax(szTemp),"SELECT COUNT(*) FROM `statystyki` WHERE `kill` >= %d", stats) // Wyciaganie z bazy count czyli w tej sytuacji twoje miejsce
...
public Topka(id)
{
	...
	format(szTemp,charsmax(szTemp),"SELECT * FROM statystyki ORDER BY kill DESC LIMIT 15")  // Wyciaganie  15 Najlepszych DESC czyli od największej do najmniejszej
